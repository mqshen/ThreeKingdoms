...
import org.goldratio.util.MutableMapWithIndifferentAccess
import org.goldratio.util.conversion.TypeConverter
...
trait AttributesMap extends Map[String, Any] with MutableMapWithIndifferentAccess[Any] {
  ...
  def as[T](key: String)(implicit mf: Manifest[T], converter: TypeConverter[Any, T]): T = {
    getAs[T](key) getOrElse (throw new SwordException("Key " + key + " not found"))
  }
