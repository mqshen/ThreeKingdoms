...
import org.goldratio.i18n.Messages
import org.json4s._
...
package object forms {
   ...
   * ValueType for the String property.
   */
  //  def filterXSSText(constraints: Constraint*): SingleValueType[String] = new SingleValueType[String](constraints: _*) {
  //    def convert(value: String, messages: Messages): String = {
  //
  //      val srcRegex = """(?i)(?s)src='(.*?)'""".r
  //      val srcQuoteRegex = """(?i)(?s)src="(.*?)"""".r
  //      //val srcQuoteRegex = new Regex("(?i)(?s)src=\".*\"")
  //
  //      def filterImgSrc(list: Iterator[Match], indictor: String): Boolean = {
  //        if (list.hasNext) {
  //          val string = list.next().group(1)
  //          if (string.startsWith(SmartLifeUtil.getImageUrl)) {
  //            filterImgSrc(list, indictor)
  //          } else {
  //            false
  //          }
  //        } else {
  ...
  //      }
  //
  //      if (filterImgSrc(srcRegex.findAllMatchIn(value), "\'") && filterImgSrc(srcQuoteRegex.findAllMatchIn(value), "\"")) {
  //        val scriptRegex = "(?i)(?s)[<\u003e]script(.*?)[>\u003c]".r
  //        scriptRegex.replaceAllIn(value, "")
  //      } else {
  //        ""
  //      }
  //    }
